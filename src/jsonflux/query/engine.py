...
def _validate_identifier(name: str) -> None:
    ...
    Table names are interpolated into SQL (``DESCRIBE {name}``,
    ``FROM {name}``), so an unrestricted name would be an injection vector.
    Requiring ``[A-Za-z_][A-Za-z0-9_]*`` keeps names safe without needing to
    quote/escape everywhere.
    """
...
class QueryEngine:
    ...
    def __init__(
        ...
        *,
        ...
        By default the underlying DuckDB connection cannot read or write the
        filesystem, cannot reach the network, and cannot load extensions, so
        SQL generated by an (untrusted) LLM can only touch the JSON you
        register.  See :class:`~jsonflux.SecurityConfig` for the full policy.

        Args:
            security: A :class:`~jsonflux.SecurityConfig`.  When ``None`` a
                secure default policy is used.
            allow_external_access: Convenience override for
                ``security.allow_external_access`` (filesystem/network access).
                Ignored when ``security`` is provided.
            memory_limit: Convenience override for ``security.memory_limit``.
                Ignored when ``security`` is provided.
            ...
            sample_scan_limit: Maximum number of top-level records scanned to
                collect example values for the LLM-facing schema.  Records are
        ...
        if security is None:
            ...
            if allow_external_access is not None:
                security.allow_external_access = allow_external_access
    ...
    def register(
        ...
        # exposed as a synthetic single ``value`` column.  Detect that so the
        # LLM-facing schema describes the real queryable column rather than the
        # raw array shape.
        root_obj = _get_root_obj(summary)
        value_wrapped = columns == ["value"] and not (
            root_obj is not None and "value" in root_obj
        )

        # Eagerly collect example values (for the LLM schema) so the raw data
        # can be discarded.  Only a stride-sampled subset of records is scanned
        # -- enough to show representative examples without walking every value.
    ...
    def query_iter(self, sql: str, batch_size: int = 1000) -> Iterator[dict[str, Any]]:
        ...
        The wall-clock ``query_timeout`` bounds each individual fetch batch
    ...
    def execute(self, sql: str) -> Any:
        """
        Execute SQL and return the raw DuckDB result.

        Note: because this hands back the DuckDB cursor for you to fetch from,
        the query timeout only covers the ``execute`` call itself, not your
    ...
    def describe_tables(
        ...
        for table_name, info in self.tables.items():
            ...
            if summary and not info.get("value_wrapped"):
    ...
    def _build_prompt(self, table_descriptions: str) -> str:
        ...
        for tname, flat in all_flat.items():
            if len(flat) >= 2:
                ...
                lines.append("```sql")
                lines.append(
                    f"SELECT {f1[0]}, {f2[0]} FROM {tname} ORDER BY {f1[0]} LIMIT 10"
                )
                lines.append("```\n")
                break
                ...
                if arr_list:
                    ...
                    if elem_kind == "object":
                        ...
                        if arr_subfields:
                            ...
                            lines.append(
                                f"FROM (SELECT unnest({arr_name}) as item FROM {tname})"
                            )
                            ...
                            lines.append("```sql")
                            lines.append(
                                f"SELECT unnest({arr_name}) as item FROM {tname}"
                            )
                            lines.append("```\n")
                    else:
                        lines.append("```sql")
        ...
        if has_multi:
            ...
            if join_hints:
                ...
                lines.append("```sql")
                lines.append(
                    f"SELECT a.*, b.* FROM {lt} a JOIN {rt} b ON a.{lc} = b.{rc}"
                )
                lines.append("```")
                if len(join_hints) > 1:
                    lines.append("\nDetected join keys:")
                ...
                lines.append("```sql")
                lines.append(
                    f"SELECT a.*, b.* FROM {t1} a JOIN {t2} b ON a.<key> = b.<key>"
                )
                lines.append("```\n")
        ...
        if has_arrays:
            ...
            lines.append(
                "2. Missing alias after UNNEST — always write `unnest(col) as item`"
            )
    ...
    def query_print(
        ...
        try:
            columns, all_rows = self._fetch(sql)
        except Exception as e:
            print(f"ERROR: {e}")
            return
